...
function removeLastFlavor(array) {
	array.pop();
	return array;
	// remove item at the end of the array using pop
	// and then eturn the array
}
...
function getFlavorByIndex(array, index) {
	return array[index];
	// return array[index] - one line of code
}
...
function removeFlavorByName(array, string) {
	for (let i = 0; i < array.length; i++) {
		if (array[i] === string) {
			array.splice(i, 1);
			// loop through array, check to see if exact string is at that index, if it is remove it
...
console.log('task 6', removeFlavorByName(originalFlavors, 'Orange Sherbet'));
...
function filterByWord(array, string) {
	...
	// need new array to push items to (look at apples example)
	// loop through the array and check to see if the item includes the string
	// if in there push it to new placeholder array
	// return the array
}
